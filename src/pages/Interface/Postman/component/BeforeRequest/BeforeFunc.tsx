...
const BeforeFunc: FC<ISelfProps> = ({ stepForm, stepInfo }) => {
  const [bodyData, setBodyData] = useState<any>();
  const [open, setOpen] = useState(false);
  useEffect(() => {
    if (stepInfo) {
      const body = stepInfo.beforeFunc;
      if (body) {
        setBodyData(body);
      } else {
        setBodyData(null);
  ...
  const handleOnChange = (value: any) => {
    if (value) {
      setBodyData(value);
      stepForm.setFieldsValue({ beforeFunc: value });
    }
  };
    ...
    <>
      ...
      >
        <ProForm form={stepForm} submitter={false}>
          <ProForm.Item name={'beforeFunc'} trigger={'onChange'}>
            <AceCodeEditor
              value={bodyData}
